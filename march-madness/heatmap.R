...
filled.contour(a, color.palette=rainbow)

tweets.csv <- read.csv(file='Data/tweets.csv', numerals='no.loss', stringsAsFactors=FALSE)
...
a <- interp(x=(data$x - 600) %% 1440, y=data$y, z=data$z)

filled.contour(a, color.palette=terrain.colors, plot.axes={ axis(1, seq(x.min, x.max, by=60))
                                                         axis(2, y.min:y.max)})
