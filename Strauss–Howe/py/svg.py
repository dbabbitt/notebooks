...
class Transformable:
    ...
    def __init__(self, elt=None):
        ...
        self.matrix = Matrix()
        self.viewport = geometry.Point(800, 600) # default viewport is 800x600
        if elt is not None:
    ...
    def bbox(self):
        ...
        return (geometry.Point(xmin,ymin), geometry.Point(xmax,ymax))
...
class Svg(Transformable):
    ...
    def parse(self, filename):
        ...
        # update viewport
        top_group.viewport = geometry.Point(width, height)
...
class Matrix:
    ...
    def __mul__(self, other):
        ...
        elif isinstance(other, geometry.Point):
    ...
    def parse(self, pathstr):
        ...
        while pathlst:
            ...
            # MoveTo
                ...
                self.items.append(geometry.MoveTo(current_pt))
            ...
            # Close Path
                l = geometry.Segment(current_pt, start_pt)
                self.items.append(l)
            ...
            # LineTo, Horizontal & Vertical line
                ...
                self.items.append(geometry.Segment(current_pt, pt))
                current_pt = pt
...
#                self.items.append(
#                    Arc(rx, ry, xrot, large_arc_flag, sweep_flag, geometry.Point(x, y)))
    ...
    def segments(self, precision=0):
        ...
        for moveTo, group in itertools.groupby(self.items,
                lambda x: isinstance(x, geometry.MoveTo)):
...
class Ellipse(Transformable):
    ...
    def __init__(self, elt=None):
        Transformable.__init__(self, elt)
        if elt is not None:
            self.center = geometry.Point(self.xlength(elt.get('cx')),
    ...
    def bbox(self):
        '''Bounding box'''
        pmin = self.center - geometry.Point(self.rx, self.ry)
        pmax = self.center + geometry.Point(self.rx, self.ry)
        return (pmin, pmax)
    ...
    def P(self, t):
        '''Return a geometry.Point on the Ellipse for t in [0..1]'''
    ...
    def segments(self, precision=0):
        ...
        while d > precision:
            for (t1,p1),(t2,p2) in zip(p[:-1],p[1:]):
                t = t1 + (t2 - t1)/2.
                d = geometry.Segment(p1, p2).pdistance(self.P(t))
                p.append((t, self.P(t)))
...
class Rect(Transformable):
    ...
    def __init__(self, elt=None):
        Transformable.__init__(self, elt)
        if elt is not None:
            self.P1 = geometry.Point(self.xlength(elt.get('x')),
                            self.ylength(elt.get('y')))

            self.P2 = geometry.Point(self.P1.x + self.xlength(elt.get('width')),
    ...
    def segments(self, precision=0):
        ...
        ret = []
        Pa = geometry.Point(self.P1.x, self.P2.y)
        Pb = geometry.Point(self.P2.x, self.P1.y)
...
class Line(Transformable):
    ...
    def __init__(self, elt=None):
        Transformable.__init__(self, elt)
        if elt is not None:
            self.P1 = geometry.Point(self.xlength(elt.get('x1')),
                            self.ylength(elt.get('y1')))
            self.P2 = geometry.Point(self.xlength(elt.get('x2')),
                            self.ylength(elt.get('y2')))
            self.segment = geometry.Segment(self.P1, self.P2)
